...
router.post('/stake/deposit', async (req, res) => {
  const { walletAddress, amount } = req.body;
  if (!walletAddress || !amount) return res.status(400).json({ error: 'missing fields' });

  try {
    const db = getDB();
    await db.collection('stakes').updateOne(
      { walletAddress },
      { $inc: { staked: amount }, $set: { updatedAt: new Date() } },
      { upsert: true }
    );
    res.json({ success: true, staked: amount });
  } catch (err) {
    res.status(500).json({ error: 'stake deposit failed', message: err.message });
  }
});
...
router.post('/stake/withdraw', async (req, res) => {
  ...
  try {
    const db = getDB();
    const userStake = await db.collection('stakes').findOne({ walletAddress });
    if (!userStake || (userStake.staked || 0) < amount) {
      return res.status(400).json({ error: 'insufficient staked balance' });
    }

    await db.collection('stakes').updateOne({ walletAddress }, { $inc: { staked: -amount }, $set: { updatedAt: new Date() } });
    const reward = amount * 0.02;
    await db.collection('users').updateOne({ walletAddress }, { $inc: { tokens: reward } });

    res.json({ success: true, withdrawn: amount, reward });
  } catch (err) {
    res.status(500).json({ error: 'withdraw failed', message: err.message });
  }
});
