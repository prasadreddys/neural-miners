...
elements.tokenBalance.addEventListener('click', async () => {
  ...
  if (!address) return;
  const res = await callAPI(`/token/balance?walletAddress=${encodeURIComponent(address)}`);
...
elements.tokenTransfer.addEventListener('click', async () => {
  ...
  const res = await callAPI('/token/transfer', 'POST', { walletAddress: state.walletAddress, to, amount: Number(amount) });
  if (res.success) {
...
elements.stakeDeposit.addEventListener('click', async () => {
  ...
  const res = await callAPI('/stake/deposit', 'POST', { walletAddress: state.walletAddress, amount: Number(amount) });
  if (res.success) {
...
elements.stakeWithdraw.addEventListener('click', async () => {
  ...
  const res = await callAPI('/stake/withdraw', 'POST', { walletAddress: state.walletAddress, amount: Number(amount) });
  if (res.success) {
